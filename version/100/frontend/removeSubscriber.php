...
if (isset($args_arr["oNewsletterEmpfaenger"]) && is_object($args_arr["oNewsletterEmpfaenger"]))
{
    if (isset($cEinstellungen_arr["api_key"]) && !empty($cEinstellungen_arr["api_key"]))
    {
        ...
        try
        {
            $cResponse_arr = $oMCAPI->lists->unsubscribe($oSync->cListId, $cEmailStruct_arr, true, (bool) $cEinstellungen_arr["send_goodbye"], (bool) $cEinstellungen_arr["send_notify"]);
        } 
        ...
        if($cResponse_arr["complete"] === true)
        {
        	...
        	Jtllog::writeLog("MailChimp: Newsletter Empfaenger erfolgreich gelöscht ".print_r($oNewsletterEmpfaenger,true), JTLLOG_LEVEL_NOTICE, false, "kPlugin", $oPlugin->kPlugin);
        }
    }
    else
    {
